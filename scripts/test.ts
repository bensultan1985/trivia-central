...
const extremesCategories = leaves.filter(leaf => leaf.path.includes('topic-extremes'));
...
leaves.forEach(leaf => {
  ...
  topLevelCategories.set(topLevel, (topLevelCategories.get(topLevel) || 0) + leaf.items.length);
});
